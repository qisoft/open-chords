...
pnpm benchmark open-sealed BUNDLE-DIRECTORY NEW-RELEASE-DIRECTORY POLICY FREEZE.json TRUSTED-AUTHORITY-PUBLIC.pem < CUSTODIAN-PRIVATE.pem
```
...
Opening requires the matching trusted signature, corpus/bundle hashes, unchanged policy file and the externally held custody key through stdin. Future freeze timestamps, changed policy/ciphertext, wrong keys and existing destinations fail. Rights are re-evaluated at opening time using the publication territory/execution-location declaration; the operator must run in that declared location. Decryption happens in private staging and produces an opening receipt with the freeze, time and current audit hash. Do not provide the released directory or unwrapped key to tuning, including after the run. The receipt is local evidence, not a central enforcement service: the custodian must retain the approved freeze/receipt and prevent a later re-tuning campaign from reusing exposed sealed data.
