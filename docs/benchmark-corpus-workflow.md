...
Hashing uses the existing Open Chords canonical serializer: recursively sorted object keys, preserved array order (chord component arrays follow the existing domain contract's lexical order), finite JSON numbers, two-space indentation and a final LF, encoded as UTF-8 and SHA-256. A stored Gold Reference is rebuilt and compared on reopening; changing raw data, hash links, adjudication or the disagreement record invalidates it. Files are immutable publications: revisions go to a new output path and have a new content hash.
