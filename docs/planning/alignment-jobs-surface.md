...
Review hardening keeps publication/storage failures retryable without opening the MFA runtime circuit. Status polling, cancellation and mutations have independent bounded slots. Invalid recovery journals preserve the desktop and recovery evidence while blocking further CPU-heavy work. Persisted anchors and recipes reuse stable identifiers; dictionary keys and transcript words share normalization, and parenthesized lyrics remain alignable. Installed tests also request Electron's normal quit lifecycle.

Windows native CI exposed an entry-point mismatch: the broker prepends `--contained-workspace`, which the worker originally rejected with exit code 64. The worker now accepts only a matching absolute workspace argument and rejects mismatches. Final installed verification and per-target size measurements are recorded in the PR and its `alignment-runtime-evidence-*` CI artifacts before readiness.
