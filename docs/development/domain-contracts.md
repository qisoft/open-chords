...
Before analysis completes, a Project has `activeView: null` and empty Analysis Revision, Edit Layer, Lyrics Document, and Lyrics Alignment arrays. This state records verified media identity and the immutable Project Range without fabricating musical assertions. `materializeEffectiveTimeline` rejects it because there is no timeline to materialize.
...
Typed operations cover Chord Identity/`N`, Chord boundaries, Beat positions, shared Bar/downbeat boundaries, meter changes, Bar split/merge, Section labels, and line/word lyric occurrence timing. A Bar split that collides with an existing Beat is rejected rather than silently replacing that Beat's stable identity. A Lyrics Alignment contains exactly one occurrence for every immutable Document line and token, in Document order; repeated text is distinguished only by its stable occurrence ID. Multi-entity structural operations materialize atomically and are rejected as a whole when the resulting timeline or alignment violates coverage, ordering, identity, or half-open interval invariants.
...
Contract 1.1 adds the persisted `replace_chord_sequence` operation for atomic chord reordering and duration changes. Both envelope and Project versions advance together. The Project Library migrates existing 1.0 Projects by publishing a new revision with unchanged musical content and retained history before editing; the original revision remains available. Safely understood future minor versions open read-only. Unknown future core semantics are rejected without recovering or overwriting the durable Head; older binaries cannot be taught a new operation by its version tag alone.

`packages/testkit/contracts/v1` is shared by TypeScript and Python. Its valid corpus includes an unanalyzed Project and a golden analyzed envelope covering pickup and truncated Bars, meter changes, an Unmetered Region, rich Chord Identities, `N`, abstention, repeated lyric occurrences, Support Claim evidence status, and explicit stale Revision selection. The invalid mutation corpus covers overlap, gaps, unstable ordering, unsafe numbers, unknown core semantics/fields, invalid stable references, cross-Revision edit/alignment references, out-of-range lyric text offsets, invalid lyric assertion states, transaction ancestry, invalid history selection, and unsupported claims without sealed evidence. The Python validator evaluates the committed generated JSON Schema before applying the same reference, timeline, alignment, ancestry, and projection invariants.
