...
The `unicode_letter_number_utf16` projection version 1.0 preserves original Unicode text, original plain-text line separators, and UTF-16 character offsets. Line/token IDs include the document identity and occurrence index; repeated words are distinct. Python validates the same UTF-16 offsets, including supplementary characters. Empty and whitespace-only physical lines remain in the immutable text but have no zero-length line occurrence.

LRC supports monotonic single timestamps per line and common descriptive header tags. Line ends derive from the next supplied onset or the Project end. Empty timed cues delimit gaps without extending words through instrumental time. Enhanced word LRC, offset directives and multiple timestamps on one line are rejected explicitly rather than guessed. SRT/WebVTT normalize CRLF and lone carriage returns consistently and preserve supplied cue intervals. YouTube json3 separator-only lines are removed when generating SRT blocks; whitespace within non-empty lines is retained. Multiline cue text preserves line breaks; the first line owns the cue interval and subsequent lines remain unmatched because separate timing was not supplied. Display markup is removed from subtitle cue text and common entities are decoded. Overlapping primary streams, empty cues and invalid intervals are rejected. No supplied line timing invents word intervals or starts MFA.
