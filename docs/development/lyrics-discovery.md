...
LRC supports monotonic single timestamps per line and common descriptive header tags. Line ends derive from the next supplied onset or the Project end. Empty timed cues delimit gaps without extending words through instrumental time. Enhanced word LRC, offset directives and multiple timestamps on one line are rejected explicitly rather than guessed. SRT/WebVTT preserve supplied cue intervals. Multiline cue text preserves line breaks; the first line owns the cue interval and subsequent lines remain unmatched because separate timing was not supplied. Display markup is removed from subtitle cue text and common entities are decoded. Overlapping primary streams, empty cues and invalid intervals are rejected. No supplied line timing invents word intervals or starts MFA.
