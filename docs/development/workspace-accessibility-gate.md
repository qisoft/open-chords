...
The automated matrix covers the main workspace, Chord Editor, invalid draft, lyrics selection, timing controls and Alignment packs dialog. It uses Tab/Enter to reach and activate controls, checks editor/picker/dialog focus restoration, and submits Russian lyrics through the keyboard. Profiles cover 1080 and 320 CSS-pixel widths, 200% desktop zoom, narrow text spacing, and forced colors with Reduced Motion. axe runs WCAG 2.2 A/AA rules plus the target-size rule. The complete results retain `incomplete` entries for human review; zero violations is not a conformance verdict. Existing renderer journeys additionally exercise playback, seek, reorder, saved edits, Undo/Redo, practice and timing corrections.
