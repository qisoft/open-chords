...
A late practice commit can invalidate a Chord Editor opened while that commit is pending. Inserting the stale-revision warning previously pushed the focused Cancel button below the viewport after focus scrolling had completed. The editor now keeps its existing focused control in view when that warning appears. A renderer regression commits through the real desktop API with Cancel focused at the viewport edge; draft invalidation, disabled Save and Reset behavior remain enforced. This does not supply native assistive-technology evidence.

The installed playback probe uses a one-second synthetic audio fixture. Polling for its transient Pause state could report that playback never started when the fixture finished between timer checks. A diagnostic 1.2-second sampling interval reproduced this false negative while event tracing recorded Pause, playing, timeline movement and ended. The probe now observes the actual button-label and timeline-style transitions before clicking Play, retains both observations and disconnects on success or the existing three-second deadline for either stage. The original Windows timeout had no event trace, so its exact scheduling remains unproven; the reproduced observation race is addressed without changing application playback or extending timeouts.

The installed Alignment cancellation check also previously polled a transient stage: a fast worker could complete before the next read of `aligning`. Cancellation now runs from the real worker stage callback after the native child launches and the Job stage is persisted. The check still requires cancelled state, rejected execution, no published Alignment and an empty temporary workspace; EN/RU publication and interrupted-workspace recovery remain exercised.
