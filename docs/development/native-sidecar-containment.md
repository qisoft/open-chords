...
Main creates a fresh AppContainer profile per attempt. Staged inputs and the writable workspace remain beneath that profile's `AC` data directory. The verified sidecar runtime is copied instead into the exact disposable `%LOCALAPPDATA%\OpenChords\ContainmentRuntime\<profile>` directory because Windows does not allow child images to execute from the profile data directory. The launcher canonicalizes both roots, requires the executable to remain beneath the runtime root, and rejects reparse points. While the contained process runs, the exact profile SID receives only non-inheriting directory traversal on `%LOCALAPPDATA%` and the two app-owned shared staging ancestors; serialized ACL updates remove those grants afterward. The disposable runtime tree receives a protected dual-principal ACL: the host user retains ownership access while the exact profile SID receives read-and-execute access only. Windows requires both the traditional user and AppContainer principals to pass the access check. The installed-artifact proof requires runtime mutation to remain denied. It never grants or mutates ACLs on the installed runtime, Source, Project Library, Model Store, or arbitrary host paths.

The frozen verifier canonicalizes Windows paths through zero-access handles shared for read, write, and delete. Sharing flags grant no access to the verifier; they prevent canonicalization from conflicting with image or directory handles already held by the frozen process. Every resolved entry is still bounded by the runtime root and checked against the immutable manifest before the protocol starts.
