...
The native launcher supplies the already canonicalized Windows runtime root after rejecting reparse points. The frozen verifier resolves every manifest entry through a zero-access handle shared for read, write, and delete. Sharing flags grant no access to the verifier; they prevent canonicalization from conflicting with image or directory handles already held by the frozen process. Every resolved entry is still bounded by the runtime root and checked against the immutable manifest before the protocol starts.
