...
The native launcher supplies the already canonicalized Windows runtime root after rejecting reparse points and starts the sidecar with that read-only root as its current directory; the writable workspace remains an explicit protocol root. The frozen verifier resolves every manifest entry through a relative zero-access handle shared for read, write, and delete, avoiding another traversal of inaccessible shared ancestors. Sharing flags grant no access to the verifier. Every handle-resolved entry is still bounded by the runtime root and checked against the immutable manifest before the protocol starts.
