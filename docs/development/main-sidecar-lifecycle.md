...
The cross-process adapter is deliberately named `createUncontainedSpawnLauncherForProof`. It launches an exact executable with an argument array, `shell: false`, a supplied working directory, a supplied environment, hidden Windows console state, and the session abort signal. It remains a protocol-only unit-test adapter. The installed proof now uses the manifest-verified production launcher: macOS XPC/App Sandbox or Windows AppContainer plus Job Object. Ubuntu Preview has a separately gated user-systemd, Landlock, and seccomp broker.

## Boundaries for the next slice

- The production adapter and installed adversarial proof are documented in [Native analysis-sidecar containment](native-sidecar-containment.md). Do not silently fall back to ordinary `spawn`.
...
- Keep graceful protocol cancellation above the platform primitive. Windows Job Objects and Ubuntu systemd scopes provide tree termination; macOS retains its documented process-group limitation while sandbox inheritance remains mandatory for every helper.
