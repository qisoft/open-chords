...
- `Добавить аккорд` is the final control inside `Аккорды в этом такте` and appends a selected `N` event; event tiles can be pointer-dragged into insertion slots before, between, or after events (including the gutters beyond the first and last tile; the marker crosses an event at its horizontal midpoint and the result is mirrored immediately on the timeline) or moved with `Alt + ←/→` from the keyboard;
