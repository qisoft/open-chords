...
3. User-supplied public video `eMm5euqjZcU`: the playlist/radio parameters were discarded and only the canonical video ID was passed to the worker. The metadata-only probe brokered six requests (791,686 response bytes), all against the exact `www.youtube.com` category. YouTube returned the same bot-check/sign-in requirement. No media was requested, no credentials were supplied, zero streams remained, and the workspace was removed.
