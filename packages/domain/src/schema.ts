...
export const StableIdSchema = z.string().regex(identifierPattern);
export const SampleFrameSchema = z.number().int().nonnegative().safe();
export const PositiveSampleFrameSchema = z.number().int().positive().safe();
const TextOffsetSchema = z.number().int().nonnegative().safe();
const HistoryPositionSchema = z.number().int().nonnegative().safe();
const PitchClassSchema = z.enum([
  "A",
  "A#",
  "Ab",
  "B",
...
export const AssertionSchema = z.discriminatedUnion("state", [
...
export const ChordIdentitySchema = z.strictObject({
...
export const ChordValueSchema = z.discriminatedUnion("kind", [
  ChordIdentitySchema,
...
export const UnmeteredRegionSchema = z.strictObject({
  ...intervalShape,
  id: StableIdSchema,
...
export const SectionRegionSchema = z.strictObject({
  ...intervalShape,
  assertion: AssertionSchema,
  id: StableIdSchema,
  label: z.enum([
...
export const KeyValueSchema = z.discriminatedUnion("kind", [
  z.strictObject({
    kind: z.literal("key"),
...
export const MusicalTimelineSchema = z.strictObject({
...
export const AnalysisRevisionSchema = z.strictObject({
...
export const LyricsDocumentSchema = z.strictObject({
...
const AlignedAssertionSchema = z.discriminatedUnion("state", [
  z.strictObject({ ...assertedShape, state: z.literal("asserted") }),
  z.strictObject({ ...assertedShape, state: z.literal("low_confidence") }),
]);

export const LyricsTimingSchema = z.discriminatedUnion("state", [
  z.strictObject({
    ...intervalShape,
...
const UserLyricsTimingSchema = z.discriminatedUnion("state", [
  z.strictObject({
    ...intervalShape,
...
export const LyricsAlignmentSchema = z.strictObject({
  analysisRevisionId: StableIdSchema,
  id: StableIdSchema,
  lineOccurrences: z.array(z.strictObject({ lineId: StableIdSchema, timing: LyricsTimingSchema })),
...
export const EditLayerSchema = z.strictObject({
  analysisRevisionId: StableIdSchema,
  id: StableIdSchema,
  transactions: z.array(EditTransactionSchema),
});

export const SupportClaimSchema = z.strictObject({
...
export const ActiveViewSchema = z.strictObject({
  analysisRevisionId: StableIdSchema,
